...
class BaseSoC(SoCCore):
    def __init__(self, board="i5", revision="7.0", sys_clk_freq=60e6, with_ethernet=False,
        ...
        mac_addr = "10:e2:d5:00:00:01"
        if mac_addr:
            mac_addr = mac_addr.split(":")
            self.add_constant("MACADR1", int(mac_addr[0], base=16))
            self.add_constant("MACADR2", int(mac_addr[1], base=16))
            self.add_constant("MACADR3", int(mac_addr[2], base=16))
            self.add_constant("MACADR4", int(mac_addr[3], base=16))
            self.add_constant("MACADR5", int(mac_addr[4], base=16))
            self.add_constant("MACADR6", int(mac_addr[5], base=16))
        ...
        # PDM mic ----------------------------------------------------------------------------------
        # Assume that PDM mic IO pin definition like
        # ("pdmmic", 0,
        ...
        self.submodules.pdmmic = pdmmic = PDMmic(platform, platform.request("pdmmic"))
